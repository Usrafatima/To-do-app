...
@patch("google.oauth2.id_token.verify_oauth2_token")
def test_verify_google_id_token_missing_client_id(mock_verify_oauth2_token):
    # Patch the settings object used in auth.py
    with patch("backend.src.auth.settings.google_client_id", None):
        token = "some_token"
        with pytest.raises(HTTPException) as exc_info:
            verify_google_id_token(token)
